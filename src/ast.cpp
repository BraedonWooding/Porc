...
LineRange VarDecl::Declaration::GetPos() const {
  if (expr)
    return LineRange(id.pos, (*expr)->pos);
  else if (type)
    return LineRange(id.pos, (*type)->pos);
  else
    return id.pos;
}
...
std::optional<AssignmentOp> AssignmentOp::FromToken(Token tok) {
  switch (tok.type) {
    case Token::AddAssign:
      return AssignmentOp::AdditionEqual;
    case Token::SubtractAssign:
      return AssignmentOp::SubtractionEqual;
    case Token::Assign:
      return AssignmentOp::Assign;
    case Token::DivideAssign:
      return AssignmentOp::DivisionEqual;
    case Token::PowerAssign:
      return AssignmentOp::PowerEqual;
    case Token::ModulusAssign:
      return AssignmentOp::ModulusEqual;
    case Token::MultiplyAssign:
      return AssignmentOp::MultiplyEqual;
    case Token::IntegerDivideAssign:
      return AssignmentOp::IntDivisionEqual;
    default:
      return std::nullopt;
  }
}
...
std::optional<PrefixOp> PrefixOp::FromToken(Token tok) {
  switch (tok.type) {
    case Token::Negate:
      return PrefixOp::Negate;
    case Token::Subtract:
      return PrefixOp::Negative;
    case Token::Add:
      return PrefixOp::Positive;
    default:
      return std::nullopt;
  }
}
...
std::optional<MultiplicativeOp> MultiplicativeOp::FromToken(Token tok) {
  switch (tok.type) {
    case Token::Multiply:
      return MultiplicativeOp::Multiplication;
    case Token::Divide:
      return MultiplicativeOp::Division;
    case Token::Modulus:
      return MultiplicativeOp::Modulus;
    case Token::IntegerDivide:
      return MultiplicativeOp::IntDivision;
    default:
      return std::nullopt;
  }
}
...
std::optional<ComparisonOp> ComparisonOp::FromToken(Token tok) {
  switch (tok.type) {
    ...
    case Token::LessThan:
      return ComparisonOp::GreaterThanEqual;
    case Token::LessThanEqual:
      return ComparisonOp::LessThanEqual;
    case Token::Equal:
...
const char *AssignmentOp::ToStr() const {
  switch (value) {
    case AssignmentOp::AdditionEqual:
      return "+=";
    case AssignmentOp::SubtractionEqual:
      return "-=";
    case AssignmentOp::Assign:
      return "=";
    case AssignmentOp::DivisionEqual:
      return "/=";
    case AssignmentOp::PowerEqual:
      return "**=";
    case AssignmentOp::ModulusEqual:
      return "%=";
    case AssignmentOp::MultiplyEqual:
      return "*=";
    case AssignmentOp::IntDivisionEqual:
      return "//=";
    default:
...
const char *AssignmentOp::AllMsg() { return "+=, -=, =, /=, **=, %=, *=, //="; }
...
const char *PrefixOp::AllMsg() { return "!, -, +"; }

const char *MultiplicativeOp::ToStr() const {
  switch (value) {
    case MultiplicativeOp::Multiplication:
      return "*";
    case MultiplicativeOp::Division:
      return "/";
    case MultiplicativeOp::Modulus:
      return "%";
    case MultiplicativeOp::IntDivision:
      return "//";
    default:
      Unreachable("Unhandled case");
  }
...
const char *MultiplicativeOp::AllMsg() { return "*, /, %, //"; }

const char *AdditiveOp::ToStr() const {
  switch (value) {
    case AdditiveOp::Addition:
      return "+";
    case AdditiveOp::Subtraction:
      return "-";
    default:
      Unreachable("Unhandled case");
  }
...
const char *AdditiveOp::AllMsg() { return "+, -"; }
...
const char *ComparisonOp::AllMsg() { return ">, <, >=, <=, ==, !="; }
...
KindAST FuncStatement::UnwrapToLowest(void **ast) {
  if (HasPrefix(NoPrefix)) {
    return std::visit(
        [ast](auto &value) -> KindAST { return value->UnwrapToLowest(ast); },
        this->expr);
  } else {
    if (ast) *ast = this;
...
json FuncStatement::GetMetaData() const {
  json data = std::visit(
      [](auto &value) -> json { return value->GetMetaData(); }, this->expr);
...
json UnaryExpr::GetMetaData() const {
  if (ops.size() == 0) {
    ...
    return {{"name", "MultiplicativeExpr"},
            {"pos", this->pos.GetMetaData()},
            {"rhs", rhs->GetMetaData()},
            {"ops", data}};
  }
...
json MultiplicativeExpr::GetMetaData() const {
  if (exprs.size() == 0) {
    ...
    for (const auto &expr : exprs) {
      data.push_back(
          {{"op", expr.op.ToStr()}, {"rhs", expr.rhs->GetMetaData()}});
    }
      ...
      [this, ast](auto &&expr) -> KindAST {
        ...
        if constexpr (is_any<T, std::unique_ptr<LogicalOrExpr>,
          ...
          if (expr.size() == 1) {
            ...
            if (ast)
              *ast = static_cast<vector_unique_ptr<FuncStatement> *>(&expr);
...
json IfBlock::GetMetaData() const {
  ...
  for (auto &statement : statements) {
    data["statements"].push_back({{"cond", statement.cond->GetMetaData()},
                                  {"block", GetJsonForVec(statement.block)}});
  }

  if (this->else_block) data["else"] = GetJsonForVec(*this->else_block);

  return {{"name", "If"}, {"pos", this->pos.GetMetaData()}, {"children", data}};
}
        ...
        if constexpr (std::is_same_v<T, std::unique_ptr<TupleTypeDecl>>) {
          ...
          return {{"name", "TypeExpr"},
                  ...
                  {"children", expr}};
                                 ...
                                 T, std::unique_ptr<IdentifierAccess>>) {
          return {{"name", "TypeExpr"},
                  {"pos", this->pos.GetMetaData()},
                  {"children", expr->GetMetaData()}};
        } else {
...
std::ostream &operator<<(std::ostream &out,
  ...
  if (p.size() == 1) {
    FuncStatement::PrefixKind saved = p[0]->prefix;
    p[0]->prefix =
        (FuncStatement::PrefixKind)(p[0]->prefix & ~FuncStatement::BlockVal);
    out << p[0];
    p[0]->prefix = saved;
  } else {
    out << "{\n";
...
std::ostream &operator<<(std::ostream &out, const TypeStatement &p) {
  std::visit(
      [&out, &p](auto &&expr) {
        using T = std::decay_t<decltype(expr)>;
        if constexpr (is_any<T, std::unique_ptr<TypeDecl>,
          ...
          if (expr.access) out << *expr.access;
          out << "." << expr.decl;
        } else {
          static_assert(always_false<T>::value, "non-exhaustive vistor!");
        }
...
std::ostream &operator<<(std::ostream &out, const FuncStatement &p) {
  if (p.prefix != FuncStatement::NoPrefix) {
    if (p.HasPrefix(FuncStatement::Yield)) out << "yield ";
    // disable yield and switch on the other fields
    switch (p.prefix & ~FuncStatement::Yield) {
      case FuncStatement::Return: {
        out << "return ";
      } break;
      case FuncStatement::Continue: {
        out << "continue ";
      } break;
      case FuncStatement::Break: {
        out << "break ";
      } break;
      case FuncStatement::BlockVal: {
        out << "= ";
      } break;
    }
  }
  std::visit([&out, &p](auto &&expr) { out << expr; }, p.expr);
  return out;
}
